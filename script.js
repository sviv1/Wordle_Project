...
const targetWordlocation = Math.random() * targetWords.length;
const targetWord = targetWords[Math.floor(targetWordlocation)];
...
function handleKeyPress(e) {
  ...
  if (e.key.match(/^[a-z]$/)) {
    // do v have one single letter between a and z
    pressKey(e.key)
    return
  }
...
function pressKey(key) {
  ...
  // added the dataset letter to the next empty tile
  nextTile.textContent = key
  // then make the text in the nexttile to insert the key
...
function flipTile(tile, index, array, guess,newColor) {
  ...
  // we have added i this is to say we want case insensitive
  setTimeout(() => {
...
function checkMatched(guess, tiles) {
  if (guess === targetWord) {
    showAlert("KYA BAAT HAI", 5000)
    danceTiles(tiles)
    stopInteraction()
    return
  }
